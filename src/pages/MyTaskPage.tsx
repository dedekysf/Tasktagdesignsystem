    ...
    if (section === "overdue") {
      ...
      if (hasDueDate) {
        ...
        if (datePattern <= 2) {
          ...
          // January 2026
          const day =
            datePattern === 3 ? 5 : datePattern === 4 ? 12 : 20;
...
export default function MyTaskPage() {
  ...
  const [currentTasks, setCurrentTasks] = useState<Task[]>(
    generateMockTasks(30, "current"),
  );
  const [overdueTasks, setOverdueTasks] = useState<Task[]>(
    generateMockTasks(5, "overdue"),
  );
  ...
  const filteredCurrentTasks = applySorting(
    applyFilters(currentTasks),
  );
  const filteredOverdueTasks = applySorting(
    applyFilters(overdueTasks),
  );
  const filteredCompletedTasks = applySorting(
    applyFilters(completedTasks),
  );
    ...
    const duplicatedTasks = tasksToDuplicate.map((task) => {
      ...
      if (options?.allMembers) {
        ...
        if (
          ...
          assigneesToCopy = task.assignees.filter(
            (assignee) => {
      ...
      const newTask: Task = {
        ...
        priority: options?.taskDetails
          ? task.priority
          : "medium",
        dueDate: options?.taskDetails ? task.dueDate : null,
        checklistCount: options?.taskDetails
          ? task.checklistCount
          : undefined,
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      ...
      <div className="h-full bg-white flex flex-col overflow-hidden">
        ...
        <div
          className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 flex flex-col gap-4 pt-[0px] pr-[16px] pb-[16px] pl-[16px]"
        ...
        >
          <TaskSection
            ...
            onAddTask={(name, priority, dueDate, assignees) =>
              handleAddTask(
                "current",
                name,
                priority,
