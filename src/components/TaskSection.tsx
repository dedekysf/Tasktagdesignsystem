...
// Note: This component requires TaskItem, InlineTaskCreation, TaskSectionHeader, and AssigneeModal
// These should be imported from their respective locations when used
...
interface TaskSectionProps {
  ...
  SectionHeaderComponent: React.ComponentType<any>;
  AssigneeModalComponent?: React.ComponentType<any>;
}
...
export function TaskSection({ 
  ...
  return (
    <div ref={drop} className={`flex flex-col bg-white rounded-lg overflow-hidden ${isExpanded && sectionType === "current" ? "flex-1 min-h-0" : "shrink-0"} ${isOver && canDrop && !isExpanded ? "ring-2 ring-primary ring-inset" : ""} ${className}`} style={{ border: '1px solid var(--grey-03)' }}>
      <SectionHeaderComponent
        title={title} 
        count={count} 
      ...
      {isExpanded && (
        <>
              ...
              <div>
                {displayedTasks.map((task, index) => (
                  <TaskItemComponent
                    ...
                    onMultiDragStart={(taskIds: string[]) => {
          ...
          {showInlineCreation && InlineTaskCreationComponent && (
            <InlineTaskCreationComponent onAddTask={onAddTask} />
          )}
        </>
      )}

      {/* Assignee Modal */}
      {assigneeModalOpen && AssigneeModalComponent && (() => {
...
export type { Task, Assignee, TaskSectionProps };